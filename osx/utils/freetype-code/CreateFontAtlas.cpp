...
int main( int argc, char **argv ) {
  ...
  // My thought process here is that each case in the switch statement will be checked and depending
  // on whether or not the useSigned variable is ever set to true

  switch(argc){
    case 7:
      if (strcmp(argv[6],"-s")) useSigned = !useSigned;
      else exit(0); // unknown argument

    case 6:
      if (!useSigned && (strcmp(argv[5],"-s")) useSigned = !useSigned;
      else glyphs = argv[5];

    case 5:
      if (!useSigned && (strcmp(argv[4],"-s")))
        useSigned = !useSigned;
      else {
        char s[50];
        strcpy(s, argv[4]);
